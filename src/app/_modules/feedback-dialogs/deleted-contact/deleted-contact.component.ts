...
@Component({
  selector: 'app-UX-deleted-contact',
  templateUrl: './deleted-contact.component.html',
  styleUrls: ['./deleted-contact.component.scss']
})
export class DeletedContactComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.firebase.currentUserFeedBack$.subscribe(active =>{
      active === 'deletedContact' ? (this.isActive = true, this.appear = true) : this.isActive = false;
      setTimeout(() => {
        this.appear = false
        this.disappear = true
      }, 3000);
